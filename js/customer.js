...
// =============================================
// 🔥 AUTH STATE LISTENER — Auto-login support
// =============================================
let _customerAuthHandled = false;

auth.onAuthStateChanged(async (user) => {
    // Only handle auth once per page load
    if (_customerAuthHandled) return;
    
    const gate = document.getElementById('auth-gate');
    
    if (user) {
        console.log('✅ Cached user detected:', user.email);
        
        // Try to load user data — with offline fallback
        try {
            let userData = null;
            
            if (navigator.onLine) {
                // Online — fetch fresh from Firebase
            ...
            if (userData && userData.role === 'customer') {
                // Mark as handled so admin.js bails out
                _customerAuthHandled = true;
                
                // Customer — show landing
                if (gate) gate.remove();
                showCustomerLanding();
                return;
            } else if (userData && userData.role === 'admin') {
                // Admin — let admin.js handle this
                console.log('👑 Admin role detected — waiting for admin.js');
                return;
            } else if (!userData) {
                // No user data (offline, no cache) — trust the auth
                console.log('📴 No cached user data — defaulting to customer landing');
                _customerAuthHandled = true;
                if (gate) gate.remove();
                showCustomerLanding();
                return;
            ...
            // Fallback: trust the auth and show customer landing
            _customerAuthHandled = true;
            if (gate) gate.remove();
            showCustomerLanding();
            return;
    ...
    // No user — but let admin.js be the one to show selection
    // (in case admin is loaded and this fires first)
    // Only show selection if admin.js hasn't handled it
    setTimeout(() => {
        if (!document.getElementById('admin-landing')?.classList.contains('active')) {
            // No one has shown a landing page — show selection
            if (gate) gate.remove();
            showSelection();
        }
    }, 100);
