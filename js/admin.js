...
// =============================================
// 🔥 SYNC OFFLINE QUEUE — Delete from queue IMMEDIATELY on success
// This is the Business Manager pattern — atomic, no dedupe needed
// =============================================
    ...
    for (const entry of queue) {
        try {
            if (entry.type === 'new_order') {
                // 🔥 new_order is handled by Firebase offline persistence.
                // The write was already queued locally when the form was submitted.
                // Firebase will flush it automatically when back online.
                // We only need to mark the signature so the listener knows to skip it
                // and remove it from our localStorage queue.
                ...
                // Clear the tracking sets after a short delay
                setTimeout(() => {
            ...
            // Roll back tracking on failure so it can retry cleanly
            if (entry.type === 'new_order') {
                _recentlySyncedIds.delete(entry.localId);
                const sig = _makeSignature(entry.data);
                if (sig) _pendingOfflineSignatures.delete(sig);
            }
            // Entry stays in queue automatically — we didn't remove it
        }
    }
...
// Keeps allCustomers[] in sync with Firebase automatically
// This is the "magic" that made Business Manager work
// =============================================
...
function setupCustomersListener() {
    ...
    // Build users map first (for phone numbers)
    database.ref('users').once('value').then(usersSnap => {
        ...
        // Now set up real-time listener on customers
        ...
        _customersListener.on('child_added', (snap) => {
            ...
            // 🔥 If offline, this entry came from Firebase's local cache — tag it
            if (!navigator.onLine) {
                customer._offline = true;
                customer._pendingSync = true;
            }
            
            // 🔥 DEDUPE against a just-synced offline entry (online only)
            if (navigator.onLine) {
            ...
            // Only add if not already there (by Firebase key)
        ...
        _customersListener.on('child_changed', (snap) => {
            ...
            if (idx !== -1) {
                // 🔥 Preserve offline flag ONLY if still offline
...
// =============================================
// LOAD CUSTOMERS — Initial one-time load
// (Real-time listener keeps it updated after this)
// =============================================
function loadCustomers() {
    ...
    Promise.all([
        ...
        // Merge offline pending entries (only those not yet in Firebase's cache)
        const offlineQueue = getOfflineQueue();
        offlineQueue.forEach(entry => {
            if (entry.type === 'new_order') {
                const sig = _makeSignature(entry.data);
                
                // Match against anything already in allCustomers (Firebase cache or otherwise)
                ...
                if (alreadyPresent) {
                    ...
                    // Auto-clean the queue
                    removeFromOfflineQueue(entry.localId);
                    return;
                }
                
                // Only add if Firebase's cache didn't already include it
                allCustomers.push({
                    ...entry.data,
                    id: entry.localId,
                    _offline: true,
        ...
        // 🔥 When offline, tag any un-tagged entries as offline
        if (!navigator.onLine) {
...
function setupAdminPaymentCheckbox(orderId, checkbox) {
    checkbox.addEventListener('change', function() {
        ...
        database.ref('customers/' + orderId).update(paymentData)
            .then(() => {
                hideLoading();
                // No need for loadCustomers() — real-time listener updates
            })
            .catch(error => {
                hideLoading();
...
function handleFormSubmit(e) {
    ...
    if (!navigator.onLine) {
        ...
        if (customerId) {
            // Edit — manual update (existing entry already in allCustomers[])
            try {
            ...
            // 🔥 New order — let Firebase's child_added listener add it
            // Firebase offline persistence queues the write locally
            try {
            ...
            // ❌ NO manual unshift — child_added will add it (tagged as offline)
        }
    ...
    // ONLINE — Firebase listener will auto-update the UI
    if (customerId) {
        ...
        .then(() => {
            ...
            // No manual loadCustomers() — real-time listener handles it
        })
        .catch(error => {
...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        try {
            ...
            const readPromise = navigator.onLine
            ...
            if (result === 'timeout') {
                const cached = localStorage.getItem('cached_user_data_' + user.uid);
                if (cached) userData = JSON.parse(cached);
            } else if (result && result.val) {
                userData = result.val();
                if (userData) {
                    localStorage.setItem('cached_user_data_' + user.uid, JSON.stringify(userData));
                }
            }
